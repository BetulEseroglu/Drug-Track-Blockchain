...
function AddMed() {
    ...
    const getStockForMedicine = (medicineName) => {
        if (selectedRetailer && selectedRetailer.drugName === medicineName) {
            console.log(`Checking stock for ${medicineName}:`, selectedRetailer); // Log the medicine details
            return selectedRetailer.stock ? parseInt(selectedRetailer.stock) : 0;
        }
        console.log(`No medicines found for retailer: ${selectedRetailer}`); // Log if no medicines are found
        return 0;
    };
    
    const handlerSubmitMED = async (event) => {
        event.preventDefault();
        console.log('Selected retailer on submit:', selectedRetailer); // Log the selected retailer on submit
        const stock = getStockForMedicine(MedName);
        console.log(`Order quantity: ${orderQuantity}, Stock available: ${stock}`); // Log the order quantity and stock available
    ...
    const handlerChangeRetailer = (event) => {
        ...
        console.log('Selected retailer:', retailer); // Log the selected retailer
    ...
    const filteredMED = Object.keys(MED).filter(key => {
        ...
        return (
            ...
            (filterStock === "" || med.stock.toString().includes(filterStock))  // Add this line to filter by stock
        );
    });
