...
function AssignRoles() {
    ...
    const [DISdrugName, setDISdrugName] = useState('');
    const [DISstock, setDISstock] = useState('');
    const [DISdrugID, setDISdrugID] = useState('');
    const [DISdrugDescription, setDISdrugDescription] = useState('');

    const [RETdrugName, setRETdrugName] = useState('');
    const [RETstock, setRETstock] = useState('');
    const [RETdrugID, setRETdrugID] = useState('');
    const [RETdrugDescription, setRETdrugDescription] = useState('');
    ...
    const handleChangeDistributorMedicine = (event) => {
        const selectedMedicine = event.target.value;
        setDISdrugName(selectedMedicine);
        setDISdrugID(medicineData[selectedMedicine]?.id || '');
        setDISdrugDescription(medicineData[selectedMedicine]?.description || '');
    };
    const handleChangeRetailerMedicine = (event) => {
        const selectedMedicine = event.target.value;
        setRETdrugName(selectedMedicine);
        setRETdrugID(medicineData[selectedMedicine]?.id || '');
        setRETdrugDescription(medicineData[selectedMedicine]?.description || '');
    };
    ...
    const handlerSubmitDIS = async (event) => {
        event.preventDefault();
        try {
            const receipt = await SupplyChain.methods.addDistributor(DISaddress, DISname, DISplace, DISdrugName, DISstock).send({ from: currentaccount });
            if (receipt) {
                loadBlockchainData();
            }
        } catch (err) {
    ...
    const handlerSubmitRET = async (event) => {
        event.preventDefault();
        console.log("Submitting retailer:", RETaddress, RETname, RETplace);
        try {
            const receipt = await SupplyChain.methods.addRetailer(RETaddress, RETname, RETplace, RETdrugName, RETstock).send({ from: currentaccount });
    ...
    return (
        <div className="container mt-4">
            ...
            <form onSubmit={handlerSubmitMAN} className="mb-4">
                ...
                <div className="mb-3">
                <input className="form-control" type="text" value={serialNumber} onChange={handleSerialNumberChange} placeholder="Seri Numarası" required />
...
<form onSubmit={handlerSubmitDIS} className="mb-4">
    ...
    <div className="mb-3">
        <select className="form-control" value={DISdrugName} onChange={handleChangeDistributorMedicine} required>
            <option value="">Select Medicine</option>
            {Object.keys(medicineData).map((name) => (
                <option key={name} value={name}>{name}</option>
            ))}
    ...
    <div className="mb-3">
        <input className="form-control" type="text" value={DISdrugDescription} placeholder="Medicine Description" readOnly />
    </div>
    <div className="mb-3">
        <input className="form-control" type="text" value={DISdrugID} placeholder="Medicine ID" readOnly />
    </div>
    <div className="mb-3">
        <input className="form-control" type="number" onChange={(e) => setDISstock(e.target.value)} placeholder="Stok Bilgisi" required />
    </div>
    <button className="btn btn-outline-success btn-sm" type="submit">Kayıt</button>
</form>
...
<form onSubmit={handlerSubmitRET} className="mb-4">
    ...
    <div className="mb-3">
        <select className="form-control" value={RETdrugName} onChange={handleChangeRetailerMedicine} required>
            <option value="">Select Medicine</option>
            {Object.keys(medicineData).map((name) => (
                <option key={name} value={name}>{name}</option>
            ))}
    ...
    <div className="mb-3">
    <input className="form-control" type="text" value={RETdrugDescription} placeholder="Medicine Description" readOnly />
</div>

    <div className="mb-3">
        <input className="form-control" type="text" value={RETdrugID} placeholder="Medicine ID" readOnly />
    </div>

    <div className="mb-3">
        <input className="form-control" type="number" onChange={(e) => setRETstock(e.target.value)} placeholder="Stok Bilgisi" required />
    </div>
    <button className="btn btn-outline-success btn-sm" type="submit">Kayıt</button>
</form>
